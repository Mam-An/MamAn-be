...
import { uploadFileToStorage, createTrackRecord, listTracks, getTrackById, updateTrackRecord, deleteTrack, getPublicUrl, getSignedUrl, generateUploadUrl, } from "./calmMusic.service.js";
...
export const uploadTrackHandler = async (req: Request, res: Response) => {
    try {
        ...
        const hasLyrics = ["true", "1", "yes"].includes(String(req.body.hasLyrics).toLowerCase());
...
export const listTracksHandler = async (req: Request, res: Response) => {
    try {
        ...
        if (hasLyrics !== undefined) {
            filters.hasLyrics = ["true", "1", "yes"].includes(String(hasLyrics).toLowerCase());
        }
